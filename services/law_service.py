...
def count_plaintiff_lawyer_company_service(plaintiff: str) -> str:
    """
    发起诉讼担任原告时，统计公司委托律师事务所的频次。
    """
    case_num_json_list = search_case_num_by_legal_document("原告", plaintiff)
    if isinstance(case_num_json_list, dict):
        case_num_json_list = [case_num_json_list]
    ...
    for item in legal_document_list:
        if "原告律师" in item and item["原告律师"]:
            ...
            matches = item["原告律师"].split(",")
            has_add = []
            for match in matches:
                if "律师事务所" in match:
                    index = match.find("律师事务所")
                    match = match[:index+"律师事务所".__len__()]
                    if match in plaintiff_lawyer_counter and match not in has_add:
                        plaintiff_lawyer_counter[match] += 1
                        has_add.append(match)
                    else:
                        plaintiff_lawyer_counter[match] = 1
    # 使用Counter来合并相同键的频次
    counter = Counter(plaintiff_lawyer_counter)
    # 然后，我们可以使用sorted函数按频次排序
    sorted_data = sorted(counter.items(), key=lambda x: x[1], reverse=False)
    # 如果你想得到一个排序后的字典，可以这样做
    sorted_dict = dict(sorted_data)
    ret = {
        "公司作为原告时委托律师事务所频次": sorted_dict
    }
    json_str = json.dumps(ret, ensure_ascii=False)
    return json_str
...
def count_defendant_lawyer_company_service(defendant: str) -> str:
    """
    面临诉讼担任被告时，统计公司委托律师事务所的频次。
    """
    case_num_json_list = search_case_num_by_legal_document("被告", defendant)
    if isinstance(case_num_json_list, dict):
        case_num_json_list = [case_num_json_list]
...
def count_lawyer_company_service(company: str) -> str:
    """
    面临诉讼时，统计公司委托律师事务所的频次。
    """
    plaintiff_lawyer_counter = {}
    case_num_json_list = search_case_num_by_legal_document("原告", company)
    if isinstance(case_num_json_list, dict):
        case_num_json_list = [case_num_json_list]
    case_num_list = [item["案号"] for item in case_num_json_list]
    legal_document_list = get_legal_document(case_num_list)
            ...
            for match in matches:
                if "律师事务所" in match:
                    ...
                    if match in plaintiff_lawyer_counter and match not in has_add:
                        ...
                        plaintiff_lawyer_counter[match] = 1
    case_num_json_list = search_case_num_by_legal_document("被告", company)
    if isinstance(case_num_json_list, dict):
        case_num_json_list = [case_num_json_list]
    case_num_list = [item["案号"] for item in case_num_json_list]
    legal_document_list = get_legal_document(case_num_list)
